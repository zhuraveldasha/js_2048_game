...
class Game {
    ...
    handleMove(grid, reverse) {
        for(let i = 0; i < this.size; i++){
            ...
            for(let j = 0; j < filtered.length - 1; j++)if (filtered[j] === filtered[j + 1]) {
                filtered[j] *= 2;
                this.score += filtered[j];
                filtered[j + 1] = 0;
                if (filtered[j] === 2048) this.gameStatus = "win";
            }
            ...
            if (!this.arraysEqual(newRow, grid[i])) grid[i] = newRow;
        }
        this.addRandomTile();
        this.checkGameOver();
    }
    ...
    addRandomTile() {
        ...
        for(let a = 0; a < this.size; a++){
            for(let b = 0; b < this.size; b++)if (this.board[a][b] === 0) empty.push([
                a,
                b
            ]);
        }
    ...
    checkGameOver() {
        if (this.board.some((row)=>row.includes(2048))) return;
        ...
        if (directions.some((fn)=>fn.call(this))) return;
